...
import TeacherProfileSettings from './ProfileSettings';
import {
...
function OverviewAnalytics({ setTab }) {
   return (
      <>
        {/* Quick Stats */}
        <div className="bg-gradient-to-r from-[#0071E3] to-[#00C6FF] rounded-[20px] p-6 md:p-8 mb-8 text-white shadow-lg">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4"><p className="text-[10px] font-medium text-white/70 uppercase tracking-wider">My Courses</p><p className="text-2xl font-bold mt-1">4</p><p className="text-[10px] text-white/60 mt-0.5">Active this term</p></div>
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4"><p className="text-[10px] font-medium text-white/70 uppercase tracking-wider">Total Students</p><p className="text-2xl font-bold mt-1">312</p><p className="text-[10px] text-white/60 mt-0.5">Across all courses</p></div>
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4"><p className="text-[10px] font-medium text-white/70 uppercase tracking-wider">Today's Classes</p><p className="text-2xl font-bold mt-1">3</p><p className="text-[10px] text-white/60 mt-0.5">Next: Physics 10:00 AM</p></div>
            <div className="bg-white/15 backdrop-blur-sm rounded-2xl p-4"><p className="text-[10px] font-medium text-white/70 uppercase tracking-wider">Pending Grading</p><p className="text-2xl font-bold mt-1">18</p><p className="text-[10px] text-white/60 mt-0.5">Assignments to review</p></div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 mb-6 flex-wrap">
          {[
            { label: 'Mark Attendance', icon: ClipboardCheck, color: 'emerald' },
            { label: 'Create Assessment', icon: FileText, color: 'violet' },
            { label: 'Schedule Class', icon: Calendar, color: 'blue' },
            { label: 'Grade Submissions', icon: CheckCircle, color: 'amber' },
          ].map((a, i) => {
            const cm = { violet: 'bg-[#0071E3]/10 hover:bg-[#0071E3]/20 text-[#0071E3]', emerald: 'bg-[#34C759]/10 hover:bg-[#34C759]/20 text-[#34C759]', blue: 'bg-blue-50 hover:bg-blue-100 text-blue-600', amber: 'bg-[#FF9500]/10 hover:bg-amber-100 text-amber-600' };
            return (
              <button key={i} className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 hover:shadow-sm ${cm[a.color]}`}><a.icon size={14} />{a.label}</button>
            );
          })}
        </div>

        {/* Today's Schedule */}
        <div className="bg-white rounded-xl border border-[#E8E8ED] shadow-sm p-6 mb-6">
          <h3 className="text-sm font-bold text-[#1D1D1F] mb-4">Today's Schedule</h3>
          <div className="space-y-3">
            {[
              { time: '09:00 - 10:30', course: 'Physics 3rd Sec', room: 'Room 201', students: 42, topic: 'Chapter 5: Optics' },
              { time: '11:00 - 12:30', course: 'Physics 3rd Sec', room: 'Room 204', students: 38, topic: 'Problem Solving Session' },
              { time: '14:00 - 15:30', course: 'Physics 2nd Sec', room: 'Lab 1', students: 35, topic: 'Lab: Refraction' },
            ].map((s, i) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-xl hover:bg-[#F5F5F7] transition-all group">
                <div className="w-16 text-[10px] font-mono font-bold text-[#0071E3] bg-[#0071E3]/10 px-2 py-1.5 rounded-lg text-center">{s.time}</div>
                <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-[#1D1D1F] truncate">{s.course}</p><p className="text-xs text-[#86868B] truncate">{s.topic} · {s.room} · {s.students} students</p></div>
                <button className="px-3 py-1.5 text-xs font-bold bg-[#0071E3]/10 text-[#0071E3] rounded-xl hover:bg-[#0071E3]/20 opacity-0 group-hover:opacity-100 transition-all">Start</button>
              </div>
            ))}
          </div>
        </div>

        {/* Two Column: My Courses + Recent Submissions */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div className="bg-white rounded-xl border border-[#E8E8ED] shadow-sm p-6">
            <h3 className="text-sm font-bold text-[#1D1D1F] mb-4">My Courses</h3>
            <div className="space-y-3">
              {[
                { name: 'Physics 3rd Sec', students: 85, progress: 72, nextClass: 'Tomorrow 10AM' },
                { name: 'Physics 2nd Sec', students: 62, progress: 58, nextClass: 'Today 2PM' },
                { name: 'Physics 1st Sec', students: 48, progress: 45, nextClass: 'Wed 9AM' },
              ].map((c, i) => (
                <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-[#F5F5F7] transition-all">
                  <div className="w-10 h-10 rounded-xl bg-[#0071E3]/10 flex items-center justify-center text-xs font-bold text-[#0071E3] shrink-0">{c.name.split(' ').map(n => n[0]).join('')}</div>
                  <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-[#1D1D1F]">{c.name}</p><p className="text-[10px] text-[#86868B]">{c.students} students · Next: {c.nextClass}</p></div>
                  <div className="text-right"><p className="text-xs font-bold text-[#0071E3]">{c.progress}%</p><div className="w-12 h-1 bg-[#E8E8ED] rounded-full mt-1 ml-auto overflow-hidden"><div className="h-full bg-[#0071E3] rounded-full" style={{width: `${c.progress}%`}}></div></div></div>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-white rounded-xl border border-[#E8E8ED] shadow-sm p-6">
            <h3 className="text-sm font-bold text-[#1D1D1F] mb-4">Recent Submissions</h3>
            <div className="space-y-3">
              {[
                { name: 'Mohamed Ali', course: 'Physics 3rd Sec', type: 'Assignment', status: 'submitted', time: '2h ago' },
                { name: 'Fatima Zahra', course: 'Physics 3rd Sec', type: 'Quiz', status: 'graded', score: 18, time: '3h ago' },
                { name: 'Youssef Samir', course: 'Physics 2nd Sec', type: 'Assignment', status: 'late', time: '5h ago' },
                { name: 'Nadia Hassan', course: 'Physics 2nd Sec', type: 'Assignment', status: 'submitted', time: 'Yesterday' },
                { name: 'Kareem Adel', course: 'Physics 3rd Sec', type: 'Quiz', status: 'graded', score: 15, time: 'Yesterday' },
              ].map((s, i) => (
                <div key={i} className="flex items-center gap-3 p-2 rounded-xl hover:bg-[#F5F5F7] transition-all">
                  <div className="w-8 h-8 rounded-lg bg-[#0071E3]/10 flex items-center justify-center text-[10px] font-bold text-[#0071E3] shrink-0">{s.name.split(' ').map(n => n[0]).join('')}</div>
                  <div className="flex-1 min-w-0"><p className="text-xs font-semibold text-[#1D1D1F] truncate">{s.name}</p><p className="text-[10px] text-[#86868B] truncate">{s.course} · {s.type}</p></div>
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${s.status === 'graded' ? 'bg-[#34C759]/10 text-[#34C759]' : s.status === 'late' ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-blue-600'}`}>{s.status === 'graded' ? `${s.score}/20` : s.status}</span>
                  <span className="text-[9px] text-[#86868B]">{s.time}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#E8E8ED] shadow-sm p-6">
          <div className="flex items-center justify-between mb-4"><h3 className="text-sm font-bold text-[#1D1D1F]">Students Needing Attention</h3><span className="text-[10px] text-[#86868B]">3 flagged</span></div>
          <div className="space-y-2.5">
            {[
              { name: 'Hossam Ibrahim', course: 'Physics 3rd Sec', issue: 'Missed 3 consecutive classes', severity: 'high' },
              { name: 'Aya Mohamed', course: 'Physics 2nd Sec', issue: 'Failing average (45%)', severity: 'high' },
              { name: 'Rana Youssef', course: 'Physics 3rd Sec', issue: 'Not submitted last 2 assignments', severity: 'medium' },
            ].map((s, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#F5F5F7] transition-all">
                <div className={`w-2 h-2 rounded-full shrink-0 ${s.severity === 'high' ? 'bg-[#FF3B30]' : 'bg-[#FF9500]'}`}></div>
                <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-[#1D1D1F]">{s.name}</p><p className="text-xs text-[#86868B]">{s.course} · {s.issue}</p></div>
                <button className="px-3 py-1.5 text-xs font-bold bg-[#0071E3]/10 text-[#0071E3] rounded-xl hover:bg-[#0071E3]/20 transition-all">Contact</button>
              </div>
            ))}
          </div>
        </div>
...
export default function TeacherDashboard() {
   ...
   const activeTabConfig = TABS.find(t => t.id === activeTab) || TABS[0];
    ...
    const isDashboard = location.pathname === '/teacher' || location.pathname === '/teacher/' || location.pathname === '/teacher/dashboard';
   ...
   const ADMIN_SIDEBAR_ITEMS = [
      ...
      { id: 'workspace', label: 'Workspace', icon: GraduationCap, path: '/teacher/workspace' },
      { id: 'meetings', label: 'Live Meetings', icon: Video, path: '/teacher/meetings' },
      { id: 'question-bank', label: 'Question Bank', icon: FileText, path: '/teacher/question-bank' },
   ...
   return (
      <div className="min-h-screen bg-[#F5F5F7] font-sans text-[#1D1D1F] flex text-base">
            ...
            <div className="flex items-center justify-between px-4 h-16 border-b border-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#86868B]">Teacher Portal</p>
                </div>
              </div>
          ...
          <div className={`${sidebarCollapsed ? 'ml-[72px]' : 'ml-[220px]'} flex-1 flex flex-col min-w-0 transition-all duration-300`}>
                 ...
                 <div className="flex items-center justify-between px-4 lg:px-6 py-3">
                   <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-[#86868B]">
                         ...
                         <span>/</span>
                         <span className="font-medium text-[#0071E3]">{isDashboard ? activeTabConfig.label : ADMIN_SIDEBAR_ITEMS.find(i => location.pathname === i.path)?.label || 'Teacher'}</span>
                      </div>
                   </div>
                     ...
                     <div className="relative">
                        ...
                        {showProfileMenu && (
                           <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-2xl border border-[#E8E8ED] shadow-xl shadow-slate-200/50 overflow-hidden z-50 animate-fadeIn">
                              <button onClick={() => { setShowProfileMenu(false); navigate('/teacher/profile-settings'); }} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#F5F5F7] transition-colors text-left">
                  ...
                  {showNotifications && (
                     <div className="absolute top-full right-4 mt-2 w-80 bg-white rounded-3xl border border-[#E8E8ED] shadow-xl shadow-slate-200/50 overflow-hidden z-50 animate-fadeIn">
                        ...
                        <div className="p-3 border-t border-slate-50 bg-[#F5F5F7]/50 text-center">
                           <button onClick={() => { setShowNotifications(false); navigate('/teacher/notifications'); }} className="text-[10px] font-bold text-[#0071E3] hover:text-[#0071E3] cursor-pointer">View All Notifications</button>
                        </div>
                     </div>
                  )}
            ...
            <div className={`flex-1 px-6 ${location.pathname === '/teacher/workspace' ? 'py-0' : 'py-6'} flex flex-col min-h-0`}>
                {location.pathname === '/teacher/workspace' ? (
                   <AdminWorkspace />
                ) : (
                   <>
                    {isDashboard ? (
                          ...
                          <AdminCourses />
                       ) : location.pathname === '/teacher/meetings' ? (
                          <AdminLiveMeeting />
                       ) : location.pathname === '/teacher/question-bank' ? (
                          <AdminQuestionBank />
                       ) : location.pathname === '/teacher/notifications' ? (
                          <AdminNotification />
                       ) : location.pathname === '/teacher/profile-settings' ? (
