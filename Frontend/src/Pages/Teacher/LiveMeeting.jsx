...
import {
  Video, Calendar, Plus, Play, X, Trash2,
  Sparkles, AlertCircle, Loader2, Search,
  Radio, History
} from "lucide-react";
...
export default function TeacherLiveMeeting() {
  ...
  const [form, setForm] = useState({ title: "", courseId: "", date: "", startTime: "", isInstant: false });
  ...
  const handleSchedule = async (e) => {
    e.preventDefault();
    if (!form.title || (!form.isInstant && !form.date)) return;
    setSubmitting(true);
    await new Promise((r) => setTimeout(r, 400));
    const startTime = form.isInstant
      ? new Date().toISOString()
      : `${form.date}T${form.startTime || "00:00"}:00`;
    const newMeeting = {
      id: Date.now(),
      title: form.title,
      course: form.courseId ? { title: `Course #${form.courseId}` } : null,
      startTime,
      status: form.isInstant ? "Live" : "Upcoming",
      meetingUrl: `https://meet.jit.si/room-${Date.now()}`,
    };
    setMeetings((prev) => [newMeeting, ...prev]);
    setShowModal(false);
    setForm({ title: "", courseId: "", date: "", startTime: "", isInstant: false });
    showToast(`Meeting "${form.title}" created`);
    setSubmitting(false);
  };
    ...
    <div className="min-h-screen bg-[#F5F5F7]">
      ...
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-16">
        ...
        <div className="flex items-center justify-between gap-4 mb-8">
          ...
          <button onClick={() => setShowModal(true)} className="shrink-0 flex items-center gap-2 px-6 py-3 bg-[#0071E3] text-white rounded-full text-sm font-semibold hover:bg-[#0056B3] active:scale-[0.97] transition-all shadow-sm">
        ...
        {activeMeetings.length === 0 && pastMeetings.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-[#86868B] mb-4">Schedule your first live meeting.</p>
            <button onClick={() => setShowModal(true)} className="px-6 py-3 bg-[#0071E3] text-white rounded-full text-sm font-semibold hover:bg-[#0056B3] transition-all">Create Meeting</button>
          </div>
        ) : (
          <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          ...
          <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl border border-[#D2D2D7]/80 relative overflow-hidden animate-admin-scaleIn" onClick={(e) => e.stopPropagation()}>
            <div className="px-6 pt-6 pb-4 border-b border-[#E8E8ED] flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-[#0071E3] flex items-center justify-center text-white shadow-sm">
                  <Video size={22} />
                </div>
                <div>
                  <h3 className="font-semibold text-xl text-[#1D1D1F]">Schedule Meeting</h3>
                  <p className="text-sm text-[#86868B]">Set up a new live session</p>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} className="p-2 rounded-xl hover:bg-[#F5F5F7] text-[#86868B] transition-colors">
                <X size={20} />
              </button>
            </div>
            <form onSubmit={handleSchedule} className="px-6 py-6 space-y-5">
              <div>
                <label className="block text-sm font-medium text-[#86868B] mb-2">Meeting Title <span className="text-[#FF3B30]">*</span></label>
                <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. React Hooks Workshop" className="w-full border border-[#D2D2D7] px-4 py-3 rounded-xl text-sm text-[#1D1D1F] focus:outline-none focus:ring-2 focus:ring-[#0071E3]/20 focus:border-[#0071E3] transition-all bg-white" />
              </div>

              <div>
                <label className="block text-sm font-medium text-[#86868B] mb-2">Course <span className="text-[#FF3B30]">*</span></label>
                <select value={form.courseId} onChange={(e) => setForm({ ...form, courseId: e.target.value })} className="w-full border border-[#D2D2D7] px-4 py-3 rounded-xl text-sm text-[#1D1D1F] focus:outline-none focus:ring-2 focus:ring-[#0071E3]/20 focus:border-[#0071E3] transition-all bg-white">
                  <option value="">Select a course...</option>
                  {["Advanced React", "AI Fundamentals", "SQL Mastery", "API Design", "TypeScript Pro", "DevOps Bootcamp"].map((c) => (
                    <option key={c} value={c}>{c}</option>
                  ))}
                </select>
              </div>

              <label className="flex items-center gap-3 p-3 rounded-xl bg-[#F5F5F7] border border-[#D2D2D7] cursor-pointer">
                <input type="checkbox" checked={form.isInstant} onChange={(e) => setForm({ ...form, isInstant: e.target.checked })} className="size-4 rounded border-[#C7C7CC] text-[#0071E3] focus:ring-[#0071E3]/20" />
                <div>
                  <p className="text-sm font-semibold text-[#1D1D1F]">Create Instant Meeting</p>
                  <p className="text-xs text-[#86868B]">Start immediately — no scheduling needed</p>
                </div>
              </label>
              ...
              {!form.isInstant && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#86868B] mb-2">Date <span className="text-[#FF3B30]">*</span></label>
                    <input required type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full border border-[#D2D2D7] px-4 py-3 rounded-xl text-sm text-[#1D1D1F] focus:outline-none focus:ring-2 focus:ring-[#0071E3]/20 focus:border-[#0071E3] transition-all bg-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[#86868B] mb-2">Time</label>
                    <input type="time" value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} className="w-full border border-[#D2D2D7] px-4 py-3 rounded-xl text-sm text-[#1D1D1F] focus:outline-none focus:ring-2 focus:ring-[#0071E3]/20 focus:border-[#0071E3] transition-all bg-white" />
                  </div>
                </div>
              )}

              <div className="flex items-center justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="px-5 py-2.5 bg-[#E8E8ED] text-[#1D1D1F] rounded-full text-sm font-semibold hover:bg-[#D2D2D7] transition-colors">Cancel</button>
                <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-[#0071E3] text-white rounded-full text-sm font-semibold hover:bg-[#0056B3] transition-all disabled:opacity-50">
                  {submitting ? <Loader2 size={16} className="animate-spin" /> : form.isInstant ? "Create & Join" : "Schedule Meeting"}
                </button>
              </div>
            </form>
          </div>
